...
namespace structures
{
	...
	public:
		...
		UnsortedSequenceTable(SortedSequenceTable<K, T>& other);
	...
	template<typename K, typename T>
	inline UnsortedSequenceTable<K, T>::UnsortedSequenceTable(SortedSequenceTable<K, T>& other) :
		SequenceTable<K, T>(new ArrayList<TableItem<K, T>*>())
	{
		for (auto item : other)
		{
			this->list_->add(new TableItem<K, T>(item->getKey(), item->accessData()));
		}
	}
