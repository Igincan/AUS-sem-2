...
namespace structures
{
	...
	inline ImplicitQueue<T>& ImplicitQueue<T>::operator=(const ImplicitQueue<T>& other)
	{
		if (this != &other)
		{
			...
			{
				throw std::out_of_range("ImplicitQueue<T>::operator=: Not enough capacity in array.");
			}
			...
			{
				int pravaCast = static_cast<int>(other.array_->size()) - other.startIndex_;
				Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, pravaCast);
				Array<T>::copy(*other.array_, 0, *array_, pravaCast, static_cast<int>(other.size_) - pravaCast);
			}
			startIndex_ = 0;
			size_ = other.size_;
		}
